...
def num_tokens_from_messages(col, messages, model='gpt-3.5-turbo-0125'):
    ...
    if model in {
        ...
        col.markdown(f'<p class="markdown-custom-1">❗ Warning: gpt-4-turbo-preview may update over time. Returning num tokens assuming gpt-4-0125-preview.</p>', 
                    unsafe_allow_html=True)
        return num_tokens_from_messages(col, messages, model='gpt-4-0125-preview')
    elif 'gpt-4' in model:
        col.markdown(f'<p class="markdown-custom-1">❗ Warning: gpt-4 may update over time. Returning num tokens assuming gpt-4-0613.</p>', 
                    unsafe_allow_html=True)
        return num_tokens_from_messages(col, messages, model='gpt-4-0613')
    elif 'gpt-3.5-turbo-16k' in model:
        col.markdown(f'<p class="markdown-custom-1">❗ Warning: gpt-3.5-turbo-16k may update over time. Returning num tokens assuming gpt-3.5-turbo-16k-0613</p>', 
                    unsafe_allow_html=True)
        return num_tokens_from_messages(col, messages, model='gpt-3.5-turbo-16k-0613')
    elif 'gpt-3.5-turbo' in model:
        col.markdown(f'<p class="markdown-custom-1">❗ Warning: gpt-3.5-turbo may update over time. Returning num tokens assuming gpt-3.5-turbo-0125</p>', 
                    unsafe_allow_html=True)
